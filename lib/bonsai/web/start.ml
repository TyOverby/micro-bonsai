...
module Arrow_deprecated = struct
  module Handle = struct
    ...
    let create ~input_var =
      ...
      { injector = Before_app_start (Queue.create ()); input_var; extra; last_extra }
    ;;
  ...
  let start_generic_poly
    ...
    let handle = Handle.create ~input_var in
    let input =
      let%map.Incr input = Incr.Var.watch input_var in
      get_app_input ~input ~inject_outgoing:(fun _ -> Effect.Ignore)
    in
