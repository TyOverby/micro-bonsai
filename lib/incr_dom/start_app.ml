...
module Visibility : sig
  ...
  type t = { mutable when_dirty : bool ref }

  let create_as_dirty () = { when_dirty = ref true }
  let mark_dirty t = t.when_dirty := true
  let is_dirty t : bool = t.when_dirty.contents
  let mark_clean t = if is_dirty t then t.when_dirty <- ref false
end
...
module Debug_flags : sig
  ...
  val init_app : app_id:string -> filter_names:String.Set.t -> debug:bool -> t
end = struct
  type t =
    { logging_filter : unit -> Logging_filter.t
    ; should_profile : unit -> bool
  ...
  let init_app ~app_id ~filter_names ~debug =
...
let start_bonsai
  ...
  (* todo: add back a "wait for document load" *)
  ...
  let cutoff =
    Incr.Cutoff.create (fun ~old_value ~new_value -> App.Model.cutoff old_value new_value)
  in
  ...
  let { Debug_flags.logging_filter; should_profile; should_debug } =
    ...
    Debug_flags.init_app ~app_id:bind_to_element_with_id ~filter_names ~debug
  in
  ...
  in
  call_viewport_changed_on_event "scroll" html_dom;
  ...
  in
  let timer_stop s = timer_stop s ~debug:(should_debug ()) ~profile:(should_profile ()) in
  (*
      ...
      let should_log_action =
        ...
        | Custom_filter filter ->
          safe_filter
            ...
            action
      in
      if should_log_action then print_s [%message "Action" (action : App.Action.t)]
  in
